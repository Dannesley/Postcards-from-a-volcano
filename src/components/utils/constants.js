export const FACTS = [
	"Sakurajima is one of the 16 'Decade Volcanoes'. Volcanoes identified as worthy of study due to their activity and proximity to populated areas.",
	"Sakurajima's summit has 3 peaks, Kita-dake (northern peak), Naka-dake (central peak) and Minami-dake (southern peak).",
	"Only one of Sakurajima's three peaks is currently active (Minami-dake, the southern peak). But each has been active in the past at some point.",
	"Sakurajima's first recorded eruption was in 708 A.D.",
	"It is believed that Sakurajima began to form around 11,000 B.C. If so, he is very old.",
	"The 1914 movie, 'The Wrath Of The Gods', was inspired by Sakurajima's famous eruption of the same year. The film began production only 16 days after the eruption.",
	"The word 'volcano' originates from the name of the Roman god of fire, 'Vulcan.",
	"80% of volcanic eruptions happen underwater.",
	"More than 80% of the earth's surface is volcanic in origin, while gaseous emissions from volcanoes helped form Earth's atmosphere.",
	"There are over 500 volcanoes on Earth. The 'Ring of Fire', a large area outlining the Pacific Ocean contains 75% of the worlds active volcanoes, and 452 volcanoes in total.",
	"The largest volcano in the galaxy, Olympus Mons (Latin for Mount Olympus), is on Mars. It is more than twice the height of Everest and is approximately the size of Italy.",
	"The largest and tallest volcano on Earth are neighbours in Hawaii. They are always able to borrow a cup of sugar from each other.",
	"The furthest point on land, from the centre of the Earth, is not the peak of Mount Everest but the volcano 'Chimborazo' in Ecuador.",
	"The eruption of Krakatoa - Part 1: The Indonesian volcanic island's eruption in 1883 exploded with the force of 13,000 times that of the modern H-bomb.",
	"The eruption of Krakatoa - Part 2: The eruption was so loud it could be heard from Perth (over 3000km away).",
	"The eruption of Krakatoa - Part 3: For a few months after the eruption, deep-red sunsets were noted around the world. It is believed this is depicted in the famous painting, 'The Scream'.",
	"You are lava'd a great magmatude",
];

const GREETINGS_MORNING = [
	"Good morning",
	"Rise n' shine!",
	"おはよう ございます",
];
const GREETINGS_AFTERNOON = [
	"Good afternoon",
	"こんにちわ",
];
const GREETINGS_EVENING = [
	"Good evening",
	"こんばんは",
];

const GREETINGS_NIGHT = [
	"Good evening",
	"Greetings night owl",
];

export const GREETINGS = { 
	MORNING: GREETINGS_MORNING,
	AFTERNOON: GREETINGS_AFTERNOON,
	EVENING: GREETINGS_EVENING,
	NIGHT: GREETINGS_NIGHT,
 };

 const QUIP_MORNING = [
	"Hope you slept well",
	"Coffee time OwO",
	"Focus on today!",
	"You were missed",
	"Have a nice day, lavaly",
	"Goodluck at work today",
 ];

 const QUIP_AFTERNOON = [
	"It's lunch time (っ˘ڡ˘ς)",
	"Keep up the good work",
 ];

 const QUIP_EVENING = [
	"Today's work is done, hooray!",
	"Time to settle in",
	"What's for dinner?",
 ];

 const QUIP_NIGHT = [
	"Pyjamas are the answer",
	"Consider a warm drink before bed",
	"Enjoy the rest of your night",
	"Get lots of rest tonight!",
 ];

 export const QUIPS = {
	 MORNING: QUIP_MORNING,
	 AFTERNOON: QUIP_AFTERNOON,
	 EVENING: QUIP_EVENING,
	 NIGHT: QUIP_NIGHT,
 }
 

